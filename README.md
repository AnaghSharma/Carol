
<p align="center"><img width=97% src="https://www.dropbox.com/s/gnwndiovrh0cc54/GitHub%20Hero.jpg?raw=1"></p>
<h2 align="center">Carol</h2>
<p align="center">Carol is a minimal and beautiful lyrics app for macOS built with SwiftUI</p>
<br>
<p align="center"><img width=100% src="https://www.dropbox.com/s/nbzaufb57q3e99h/GitHub%20Screenshot.png?raw=1"></p>
<br>

## Features
* Automatically identifies the currently playing song and shows its lyrics.
* One-touch access to lyrics straight from the menu bar.
* Supports both Apple Music and Spotify.
* Minimal design with support for Album artworks.
* Adapts to system's light/dark theme.
* Light in size.

## How to use
#### Requirements
macOS 10.15 (Catalina) and above.
#### Download
[Download the latest release from here.](https://github.com/AnaghSharma/Carol/releases)
#### Install
Unzip the `Carol.zip` file and copy `Carol` to your Applications folder.

#### Launch
Launch the app from the Applications folder. It will appear in your menu bar. Provide required permissions on first launch and Enjoy.
<br>

## Donate
Buy me a coffee to support the development of this project. SwiftUI has much more to offer that can be implemented in this project.

<a href="https://www.buymeacoffee.com/anagh"><img src="https://img.buymeacoffee.com/button-api/?text=Buy me a coffee&emoji=&slug=anagh&button_colour=FFDD00&font_colour=000000&font_family=Poppins&outline_colour=000000&coffee_colour=ffffff"></a>

## Contributing
Thanks for showing interest in this project and its development. If you think this project can be improved, you are welcome to contribute. Please go through the [CONTRIBUTING.md](https://github.com/AnaghSharma/Carol/blob/master/CONTRIBUTING.md) for contributing instructions.

## Contact
[Follow me on Twitter](https://twitter.com/AnaghSharma)

## Disclaimer
This project is for learning purpose only. If you want to learn macOS development and SwiftUI technologies, feel free to use this project. Please do not sell/re-sell the project or its content as the lyrics are copyrighted material and are properties of their respective owners.
